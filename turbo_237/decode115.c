...
void main()
{
    ...
    for (int i = 0; i < FRAME_LENGTH; i++) {
        fscanf_s(input_file, "%d", &trafficflow_source[i]);//把源数据（int）存在trafficflow_source数组中
    }
    fclose(input_file);

    //// 进行编码并将编码结果保存到 output115.txt
    FILE* output_file;
    errno_t err_output = fopen_s(&output_file, "output116.txt", "w");
    if (err_output != 0) {
    ...
    // 设置固定的信噪比（无用操作，只是解码函数中需要，后续可以根据实际中测算出来的信噪比进行修改）
    EbN0dB = 1.0;
    ...
    // 编码过程，这里有个疑问，现在没想出来为什么删掉这一行会报错，但是保留就没事，所以我就保留了，
    //但是实际上用于编码的数据是从编码代码产生的输出文件中读出来的，也就是下面的代码，而非这个文件里的TurboCodingTraffic（）的输出
    ...
    for (int i = 0; i < 2 * FRAME_LENGTH; i++) {
        fscanf_s(input_coded_file, "%f", &trafficflow_for_decode[i]);//读出编码后的数据存在trafficflow_for_decode数组（float）中
    }
